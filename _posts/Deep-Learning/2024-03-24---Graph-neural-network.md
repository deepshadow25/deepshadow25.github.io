...
- $W_l$은 $l$층에서 $l+1$층으로 변환할 때 가중치 행렬이다
...
- $V$ : vertex set (노드 집합)
- $\bold{A}$ : 인접행렬 (이진으로 가정)
- $\bold{X} \in \mathbb{R}^{m \times |V|}$ : node features의 행렬
- $v$ : 집합 $V$에 속하는 노드
    - $N(v)$ : $v$ 이웃 노드들의 집합
...
$$
\mathbf{h}_v^{l+1} = \sigma(W_l \sum_{u\in N(v)} \frac{\mathbf{h}_u^{(l)}}{|N(v)|} + \mathbf{B}_l \mathbf{h}_v^{l}), \forall l \in \{ 0, ..., L-1\}
$$
...
$\mathbf{h}_v^{(l)}$ : 노드 $v$의 레이어 $l$에서의 임베딩
